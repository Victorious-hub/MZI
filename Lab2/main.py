...
class STB:
    ...
    def encryption(self, m):
        """m: list of 16 ints (bytes). returns list of 16 ints"""
...
def int_to_bytes_list(n, length):
    return [(n >> (8 * (length - 1 - i))) & 0xFF for i in range(length)]

def split_blocks_bytes(data_bytes, block_size=16):
    return [list(data_bytes[i:i+block_size]) for i in range(0, len(data_bytes), block_size)]

def xor_bytes_len(a, b, length):
    return [ (a[i] ^ b[i]) & 0xFF for i in range(length) ]

def cfb_encrypt(plaintext_bytes, key_bytes, S_bytes):
    blocks = split_blocks_bytes(plaintext_bytes, 16)
    stb = STB(list(key_bytes))
    Y_prev = list(S_bytes)
    cipher_blocks = []

    for Xi in blocks:
        keystream = stb.encryption(Y_prev)
        L = len(Xi)
        Yi = xor_bytes_len(Xi, keystream, L)
        cipher_blocks.append(bytes(Yi))
        Y_prev = Yi  # Y_i
    return b"".join(cipher_blocks)

def cfb_decrypt(ciphertext_bytes, key_bytes, S_bytes):
    blocks = split_blocks_bytes(ciphertext_bytes, 16)
    stb = STB(list(key_bytes))
    X_prev = list(S_bytes)  # X0 = S
    plain_blocks = []

    for Xi in blocks:
        keystream = stb.encryption(X_prev)
        L = len(Xi)
        Yi = xor_bytes_len(Xi, keystream, L)
        plain_blocks.append(bytes(Yi))
        X_prev = Xi
...
def main():
    ...
    print(f"Generated sync package (hex): {S.hex()}")

    ct = cfb_encrypt(data, key, S)
    with open("encrypted.bin", "wb") as f:
        f.write(S + ct)

    with open("encrypted.hex.txt", "w", encoding="utf-8") as f:
        f.write((S + ct).hex())
    ...
    S_read = content[:16]
    ct_read = content[16:]
    recovered = cfb_decrypt(ct_read, key, S_read)

    with open("decrypted.text", "wb") as f:
        f.write(recovered)

    print("Исходный размер:", len(data))
    print("Шифротекст размер:", len(ct))
    print("Восстановлено совпадает с исходным:", recovered == data)
