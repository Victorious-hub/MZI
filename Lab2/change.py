...
def main():
    ...
    with open("encrypted_simple_change.txt", "w", encoding='utf-8') as f:
        for item in encrypted_result:
            f.write(str(item) + ' ')

    with open("decrypted_simple_change.txt", "w", encoding='utf-8') as f:
...
# Каждый блок шифруется отдельно, с новым случайным ключом → это на самом деле ECB, только ключ меняется для каждого блока.

# Нет обратной связи между блоками.

# Padding:

# Текст выравнивается нулями → простой способ сделать длину кратной 16.

# Проблемы:

# Генерация нового ключа для каждого блока делает шифротекст нестабильным (его нельзя расшифровать повторно без хранения всех ключей).